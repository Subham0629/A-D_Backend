...
async function createData(req, res) {
  try {
    ...
    const existingWarehouse = await WarehouseSchema.findOne({
      warehouse_name: {
        $regex: new RegExp("^" + lowerCaseWarehouseName + "$", "i"),
      },
    });
...
async function deleteData(req, res) {
  try {
    ...
    const dataRemoved = await WarehouseSchema.deleteOne({
      _id: new ObjectID(id),
    });
    res
      .status(200)
...
async function getDetails(req, res) {
  try {
    ...
    const recordDetail = await WarehouseSchema.findOne({
